...
public class Chat {
	...
	public static boolean checkChat(User receiver, User sender) {

		int n = -10;
		for (int i = 0; i < sender.getMessages().size(); i++) {
	    	if (sender.getMessages().get(i).get(1).get(1) == receiver.getName()) {
				n = i;
	           	break;
			}
		}
	...
	public static void sendMessage(User receiver, User sender, boolean flag) {
		...
		if (flag == true) {
			...
			for (int i = 0; i < receiver.getMessages().size(); i++) {
				if (receiver.getMessages().get(i).get(1).get(1) == sender.getName()) {
					r = i;
				  	break;
				}
			}
